...
//@EnableScheduling
@Service
public class AssessBuildServiceImpl {
    //
//    @Autowired
...
//    @Scheduled(cron = "0 0/2 * * * ?")//0 /1 * * ?  每隔1分钟执行一次
    public Response buildCompreAssess(int khlx,
        ...
        if (khid != null && khid != "") {
            assessStaticDao.deleteCommonSorceData(khid);
            assessStaticDao.deleteCommonDetailData(khid);
            assessStaticDao.deleteCommonInfoData(khid);
        }
        //省
        ...
        if (assessConfigBeans != null && assessConfigBeans.size() > 0) {
            ...
            if (dw_code != null && dw_code != "") {
                createInfoTableData(dw_code, dw_lb, assessConfigBeans, khkssj, khjssj, khlx);
            } else {
        ...
        return new Response().success("生成成功");
    }
